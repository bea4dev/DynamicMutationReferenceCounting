 ...
 * よって、それらのオブジェクトのフィールドに新しいオブジェクトの参照を持たせる場合はその新しいオブジェクトとそれに連なるすべてのオブジェクトの
 * is_mutex を true に設定してから操作を行えば、is_mutex を読むことで複数のスレッドからアクセスされうるかどうかを少ない実行時コストで判定することができる。
